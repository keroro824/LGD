...
LogitSgd::LogitSgd(double** train_data, double** table_data, double** train_label, double** test_data, double** test_label, int trainNum, int testNum, int dim, double conv, double lr, int epoch, double decayrate, double reg, int check, std::string outputFile)
{
...
LogitSgd::LogitSgd(LSH *Algo, double** train_data, double** table_data, double** train_label, double** test_data, double** test_label, int trainNum, int testNum, int dim, double conv, double lr, int epoch, double decayrate, double reg, int check, int srpratio, int K, int L, std::string outputFile)
{
    _Algo = Algo;
    _train_data = train_data;
    _table_data = table_data;
...
void LogitSgd::LogitrandomData(int* index ,int size)
{
...
double LogitSgd::Logitdotproduct(double* a, double* b, int start, int size)
{   
    double total = 0.0;
...
int LogitSgd::Logitpredict(string outputFile, int time, int iter)
{
    cout << "Logitpredicting\n";
    ...
    {
        p_res = Logitdotproduct(_train_data[i], _wv, 0, _dim);
    ...
    {
        p_res = Logitdotproduct(_test_data[i], _wv, 0, _dim);
...
int LogitSgd::LogitSGDUpdate(int ada)
{
	//initialize timer
    ...
    //initial Logitprediction
    Logitpredict(_outputFile, 0,cur_iter);
    ...
    while (cur_iter < _epoch) {    
	    ...
	    double p_res, tmp;
	    //shuffle data
	    LogitrandomData(index, _trainNum);
	    ...
	    for (int i = 0; i < _trainNum; i++) {
            ...
            //compute gradient
            double tmp = Logitdotproduct(td, _wv, 0, _dim) - cur_label;
	    ...
	    cur_iter++;
	    Logitpredict(_outputFile, t, cur_iter);
	}

	return 0;
}

void LogitSgd::Logitfreeze(SignedRandomProjection *srp, int iter, int train){
    ...
    for (int n=0; n<_trainNum; n++){
        ...
        for (int f=0; f<_dim; f++){
            double tmp_gradient = _train_data[n][f]*(Logitdotproduct(_train_data[n], _wv, 0, _dim) - _train_label[n][0]);
            if (n==0){
    ...
    for (int batch=0; batch< 1000; batch++){
        ...
        for (int sam =0; sam< sizz*(batch+1); sam++){
            ...
            double thetax = Logitdotproduct(td, _wv, 0, _dim)-cur_label;
            ...
            sgd_error += 1- acos(Logitdotproduct(sgd_grad, truegrad, 0, _dim)/_trainNum/sqrt(true_gradient_norm*sgd_norm))/3.141592653;
            ...
            thetax = Logitdotproduct(td, _wv,0 , _dim)-cur_label;

            double cp_comp = Logitdotproduct(_table_data[samid], _wv, 0, _dim) - _table_data[samid][_dim];
            ...
            lsd_error += 1- acos(Logitdotproduct(lsd_grad, truegrad, 0, _dim)/_trainNum/sqrt(true_gradient_norm*lsd_norm))/3.141592653;
...
int LogitSgd::LogitLSDUpdate(int ada)
{
	auto start_timer = std::chrono::steady_clock::now();
    auto end_timer = std::chrono::steady_clock::now();
    auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(end_timer - start_timer);
...
int LogitSgd::LogitGDUpdate(int ada)
{
	//initialize timer
	auto start_timer = std::chrono::steady_clock::now();
    auto end_timer = std::chrono::steady_clock::now();
